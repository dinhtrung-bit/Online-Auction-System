...
/**
 * ItemFactoryTest — thiết kế theo giáo trình UET.CS2043
 *
 * Kỹ thuật:
 ...
 *   - Vehicle server-side constructor đã đổi từ
 *     Vehicle(id, name, price, desc, brand, year, engine)
 *     thành Vehicle(id, name, price, desc) — ItemFactory không truyền
 *     brand/year/engine nữa.
 *   - Các test khác không thay đổi vì ItemFactory.createItem() API không đổi.
 */
public class ItemFactoryTest {
    ...
    @Test @DisplayName("[BVA] Category 'vehicle' viết thường — factory phải normalize")
    void testCreate_LowercaseVehicle_BVA() {
        Item item = ItemFactory.createItem("vehicle", 4, "Bike", new BigDecimal("15000"), "d");
        assertInstanceOf(Vehicle.class, item);
    }
    ...
    @Test @DisplayName("[EP] Vehicle: getCategoryInfo() trả về 'VEHICLE'")
    void testCreate_VehicleCategoryInfo_EP() {
        Item item = ItemFactory.createItem("VEHICLE", 10, "Truck", new BigDecimal("500000"), "xe tải");
        assertEquals("VEHICLE", item.getCategoryInfo());
    }

    @Test @DisplayName("[EP] Vehicle: seller ban đầu là null (chưa được set)")
    void testCreate_VehicleSellerNull_EP() {
        Item item = ItemFactory.createItem("VEHICLE", 10, "Truck", new BigDecimal("500000"), "xe tải");
        // ItemFactory không gắn seller; DAO gọi setSeller() sau khi tạo
