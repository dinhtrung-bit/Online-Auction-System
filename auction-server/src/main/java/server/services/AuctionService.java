...
public class AuctionService {
    ...
    public String handleBidRequest(Long roomId, Bidder bidder, double amount) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                // ✅ Dùng BidRecord để lưu DB (không dùng BidMessage)
                bidDAO.insert(new BidRecord(roomId.intValue(), bidder.getUserId(), bidAmount));
    ...
    public String cancelAuctionBySeller(int auctionId, int sellerId) throws Exception {
        ...
        if (room.getStatus() == AuctionStatus.PAID
                || room.getStatus() == AuctionStatus.FINISHED) return "Phiên đã kết thúc/thanh toán, không thể hủy!";
    ...
    public void registerAutoBid(int auctionId, Bidder bidder,
        ...
        // Dùng constructor có sẵn — không cần tạo AuctionRoom rỗng làm workaround
    ...
    /**
     * Lấy lịch sử bid kèm username (dùng cho AuctionRequestHandler).
     * bidDAO.getBidHistoryByAuctionRoomId trả về List<BidRecord>.
     */
