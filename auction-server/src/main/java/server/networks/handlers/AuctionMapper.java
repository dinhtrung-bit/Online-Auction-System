...
/**
 * AuctionMapper — Chuyển đổi AuctionRoom thành Map để serialize thành JSON response.
 *
 * Tách ra khỏi AuctionRequestHandler để:
 *   - Dễ test riêng (không cần mock service)
 *   - Dùng lại ở nhiều handler nếu cần (Admin, Report...)
 *   - Giảm kích thước AuctionRequestHandler
 */
public final class AuctionMapper {
    ...
    /**
     * Map chi tiết cho AUCTION_DETAIL endpoint — bao gồm thêm secondsLeft, bidCount.
     */
    ...
    /**
     * Parse AuctionStatus từ string, hỗ trợ cả tiếng Anh lẫn tiếng Việt.
     * Trả về null nếu không nhận ra (coi như "tất cả").
     */
    public static AuctionStatus parseStatus(String statusStr) {
        ...
        try {
            ...
            return switch (statusStr.trim().toUpperCase()) {
